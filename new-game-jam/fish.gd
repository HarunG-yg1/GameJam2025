...
func move(direct,modifier):
		if (direct.length()) > 0.0:
			if (((velocity-(grav_dir*velocity.length())).length()) <  MAX_SPEED) and statemachine.state == "falling" || (velocity.length()) <  MAX_SPEED :
				...
				if abs(modifier) == 1 and (velocity.normalized() - direct.normalized()).length() >1:
					...
					velocity += direct * INITIAL_SPEED *modifier
				elif statemachine.state != "jumpin" and  statemachine.state != "attack" and statemachine.state != "falling" :
