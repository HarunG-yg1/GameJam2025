...
func Process(_delta):
		...
		if attack_time < .4:
			if (enemy.player.global_position-enemy.global_position).length() < 16 and !enemy.player.hitting and enemy.hitting and enemy.player.finish_run and enemy.player.hp_and_stuff.damaged_timer <= 0 and enemy.was_hit < 0.1:#and (enemy.sprite.position -enemy. player.position).length() < 400:
				print("yo")
